...
void setup()
{
    ...
    lcd.setCursor(0,1); lcd.print("     BrewFuckers      ");
...
//    if (millis() > milisegundos  && Input > Setpoint){ //cuando alcance el tiempo de duracion de la rampa pasa al siguiente ciclo
//      ciclo = ciclo+1;
//      tC=0;
//    
//  }
       if (millis() > milisegundos  && !array_flag[ciclo]){ //cuando alcance el tiempo de duracion de la rampa pasa al siguiente ciclo
      ciclo = ciclo+1;
      tC=0;
 ...
 if (millis()>tiempoActual + 500){
      Input = thermocouple.readCelsius(); // se recoje una medida de Temp cada medio segundo
//    Input = Input + 14; //error apreciado
//    Serial.print("Temp: ");
//    Serial.print(Input);
//    Serial.print("   ");
//    Serial.print("ciclo:  ");
...
digitalWrite(2, LOW);
  ...
  if (Input < Setpoint && estado == LOW){
    digitalWrite(2, HIGH);
    estado = HIGH;
  }
  if (Input < (Setpoint-2) && estado == HIGH){
    digitalWrite(2, HIGH);
  }
} 
 if (ciclo >= n_ciclos){
